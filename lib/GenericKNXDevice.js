...
class GenericKNXDevice extends Homey.Device {
  ...
  addKNXEventListeners(settings) {
    ...
    if (this.hasCapability('target_temperature')) {
      this.knxInterface.addKNXEventListener(this.getStatusAddress('ga_temperature_target'),
        this.KNXEventHandler);
    }
    if (this.hasCapability('hvac_operating_mode')) {
      this.knxInterface.addKNXEventListener(this.getStatusAddress('ga_hvac_operating_mode'),
        this.KNXEventHandler);
    }
    if (this.hasCapability('knx_hvac_controller_mode')) {
      this.knxInterface.addKNXEventListener(this.getStatusAddress('ga_hvac_controller_mode'),
        this.KNXEventHandler);
    }
    ...
    if (this.hasCapability('knx_fan_speed')) {
      this.knxInterface.addKNXEventListener(this.getStatusAddress('ga_fan_speed'), this.KNXEventHandler);
    }
    if (this.hasCapability('knx_fan_auto_mode')) {
      this.knxInterface.addKNXEventListener(this.getStatusAddress('ga_fan_auto_mode'), this.KNXEventHandler);
    }
    if (this.hasCapability('windowcoverings_set')) {
      this.knxInterface.addKNXEventListener(this.getStatusAddress('ga_height'), this.KNXEventHandler);
    }
  }
  ...
  removeKNXEventListeners(settings) {
    ...
    if (this.hasCapability('target_temperature')) {
      this.knxInterface.removeKNXEventListener(this.getStatusAddress('ga_temperature_target'),
        this.KNXEventHandler);
    }
    if (this.hasCapability('measure_temperature')) {
      if (this.settings.ga_sensor) {
    ...
    if (this.hasCapability('hvac_operating_mode')) {
      this.knxInterface.removeKNXEventListener(this.getStatusAddress('ga_hvac_operating_mode'), this.KNXEventHandler);
    }
    if (this.hasCapability('knx_hvac_controller_mode')) {
      this.knxInterface.removeKNXEventListener(this.getStatusAddress('ga_hvac_controller_mode'), this.KNXEventHandler);
    }
    if (this.hasCapability('knx_fan_speed')) {
      this.knxInterface.removeKNXEventListener(this.getStatusAddress('ga_fan_speed'), this.KNXEventHandler);
    }
    if (this.hasCapability('knx_fan_auto_mode')) {
      this.knxInterface.removeKNXEventListener(this.getStatusAddress('ga_fan_auto_mode'), this.KNXEventHandler);
    }
    if (this.hasCapability('windowcoverings_set')) {
      this.knxInterface.removeKNXEventListener(this.getStatusAddress('ga_height'), this.KNXEventHandler);
    }
  }
  ...
  async readSettingAddress(keys) {
    ...
    return Promise.all(settingKeys.map((settingKey) => {
      ...
      .catch((error) => {
        this.error('readsetting_failed', error);
      });
  }
  ...
  /**
   *  Used to check if a separate status address has been supplied.
   *  Will return the supplied address with '_status' appended if it exists,
   *  otherwise the original address will be used.
   *  Example: Setting through ga_temperature_target, status in ga_temperature_target_status
   */
  getStatusAddress(addressName) {
    const statusAddressName = `${addressName}_status`;
    if (typeof this.settings[statusAddressName] === 'string' && this.settings[statusAddressName] !== '') {
      return this.settings[statusAddressName];
    }
    return this.settings[addressName];
  }
